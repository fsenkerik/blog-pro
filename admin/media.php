...
$typeCounts = ['image' => 0, 'document' => 0, 'video' => 0, 'audio' => 0];
if ($totalCount > 0) {
    foreach ($media->getAll($search, 1, max(1, $totalCount)) as $countItem) {
        $countType = mediaType($countItem['original_name'] ?? $countItem['filename'] ?? '');
        if (isset($typeCounts[$countType])) {
            $typeCounts[$countType]++;
        }
    }
}
...
<div class="app">
  <aside class="side">
    ...
    <div><div class="nav-label">Workspace</div><nav class="nav">
      ...
      <a href="<?= ADMIN_URL ?>posts.php"><svg class="ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M4 4h12l4 4v12H4z"/><path d="M16 4v4h4"/><path d="M8 13h8M8 17h5"/></svg>Příspěvky<span class="count"><?= $totalAll ?></span></a>
      <a href="<?= ADMIN_URL ?>media.php" class="active"><svg class="ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M4 16V6a2 2 0 0 1 2-2h8l6 6v6a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2z"/><circle cx="9" cy="11" r="1.5"/><path d="m4 18 5-5 5 5 3-3 3 3"/></svg>Média<span class="count"><?= $totalMedia ?></span></a>
    </nav></div>
        ...
        <aside class="filters">
          <div class="filt">
            ...
            <div class="filt-list">
              ...
              <a href="?type=image&search=<?= urlencode($search) ?>" class="filt-item <?= $typeFilter==='image'?'on':'' ?>"><svg class="ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="m21 15-5-5L5 21"/></svg>Obrázky<span class="ct"><?= $typeCounts['image'] ?></span></a>
              <a href="?type=document&search=<?= urlencode($search) ?>" class="filt-item <?= $typeFilter==='document'?'on':'' ?>"><svg class="ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>Dokumenty<span class="ct"><?= $typeCounts['document'] ?></span></a>
              <a href="?type=video&search=<?= urlencode($search) ?>" class="filt-item <?= $typeFilter==='video'?'on':'' ?>"><svg class="ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2"/></svg>Videa<span class="ct"><?= $typeCounts['video'] ?></span></a>
              <a href="?type=audio&search=<?= urlencode($search) ?>" class="filt-item <?= $typeFilter==='audio'?'on':'' ?>"><svg class="ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>Zvuk<span class="ct"><?= $typeCounts['audio'] ?></span></a>
            </div>
          </div>
        </aside>

...
// Client-side type filter note: type filter applied via JS for simplicity
$mediaItems = $allItems;
...
.storage-num .of{color:var(--muted);font-size:12px}
...
.thumb-doc{background:linear-gradient(180deg,#fff,#f3f4f6)}
  ...
  <aside class="side">
    ...
    <div>
      ...
      <nav class="nav">
        ...
        <a href="<?= ADMIN_URL ?>media.php" class="active">
  ...
  <main class="main">
    <div class="topbar">
      ...
      <div class="search">
        ...
        <input type="text" placeholder="Hledat napříč CMS…">
        <span class="kbd">⌘K</span>
      </div>
      <div class="top-actions">
        <a href="<?= ADMIN_URL ?>add_post.php" class="btn btn-primary btn-sm">
    ...
    <div class="content">
      ...
      <div class="storage">
        <div>
          <div class="storage-bar"><div class="storage-fill" id="storageFill" style="width:42%"></div></div>
          <div class="storage-meta"><?= $totalCount ?> souborů celkem</div>
        </div>
        <div class="storage-num">
          <strong id="storageSize"><?= $stats['total_size_formatted'] ?></strong>
        </div>
      </div>
      ...
      <div class="media-shell">
        <aside class="filters">
          <div class="filt">
            <h4>Typ souboru</h4>
            <div class="filt-list">
              <a href="?search=<?= urlencode($search) ?>" class="filt-item <?= $typeFilter === 'all' || !$typeFilter ? 'on' : '' ?>">
                <svg class="ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="10"/></svg>
                Vše <span class="ct"><?= $totalCount ?></span>
              </a>
              <a href="?type=image&search=<?= urlencode($search) ?>" class="filt-item <?= $typeFilter === 'image' ? 'on' : '' ?>">
                <svg class="ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="m21 15-5-5L5 21"/></svg>
                Obrázky
              </a>
              <a href="?type=document&search=<?= urlencode($search) ?>" class="filt-item <?= $typeFilter === 'document' ? 'on' : '' ?>">
                <svg class="ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                Dokumenty
              </a>
              <a href="?type=video&search=<?= urlencode($search) ?>" class="filt-item <?= $typeFilter === 'video' ? 'on' : '' ?>">
                <svg class="ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2"/></svg>
                Videa
              </a>
              <a href="?type=audio&search=<?= urlencode($search) ?>" class="filt-item <?= $typeFilter === 'audio' ? 'on' : '' ?>">
                <svg class="ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>
                Zvuk
              </a>
            </div>
          </div>
        ...
        <div>
          <form action="" method="GET" id="searchForm">
            ...
            <div class="toolbar">
              <div class="tb-search">
                <svg class="tb-search-ico" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg>
                <input type="text" name="search" placeholder="Hledat v médiích…" value="<?= e($search) ?>" onchange="this.form.submit()">
              </div>
              <div class="tb-divider"></div>
              <span class="tb-meta" id="tbMeta"><?= $totalCount ?> souborů</span>
            </div>
          </form>

          <div class="selbar" id="selbar">
            <span class="selbar-count"><strong id="selCount">0</strong> vybráno</span>
            <div class="selbar-actions">
              <button class="btn btn-ghost btn-sm" style="color:var(--danger)" onclick="openDeleteModal()">
              ...
              <button class="btn btn-ghost btn-sm" id="selClear">Zrušit výběr</button>
            </div>
          </div>
          ...
          <div class="upload-strip" id="uploadStrip">
            ...
            <div>
              <div class="upload-text">Přetáhněte soubory sem nebo klikněte pro výběr</div>
              <div class="upload-sub">JPG, PNG, WebP, PDF · max. 50 MB / soubor</div>
            </div>
          ...
          <?php if (empty($mediaItems)): ?>
            <div style="text-align:center;padding:60px 20px;color:var(--muted)">
              <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="opacity:.3;display:block;margin:0 auto 16px"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="m21 15-5-5L5 21"/></svg>
              <p>Žádná média<?= $search ? ' pro &ldquo;'.e($search).'&rdquo;' : '' ?>.</p>
            </div>
          <?php else: ?>
            <div class="grid-m" id="mediaGrid">
              ...
              <div class="card-m" data-id="<?= $item['id'] ?>" data-type="<?= $mtype ?>">
                <div class="thumb <?= $mtype === 'document' ? 'thumb-doc' : ($mtype === 'video' ? 'thumb-vid' : ($mtype === 'audio' ? 'thumb-aud' : '')) ?>" style="<?= $mtype === 'image' ? 'background:'.$grad.';' : '' ?>">
                  <?php if ($mtype === 'image'): ?>
                    <img src="<?= BASE_URL . $item['path'] ?>" alt="<?= e($item['original_name']) ?>" loading="lazy" onerror="this.style.display='none'">
                  ...
                  <?php else: ?>
                    <svg width="32" height="42" viewBox="0 0 24 30" fill="none" stroke="currentColor" stroke-width="1.5" style="color:var(--secondary)"><path d="M14 2H6a2 2 0 0 0-2 2v22a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                  <?php endif; ?>
                ...
                <div class="meta-m">
                  <div class="meta-m-name" title="<?= e($item['original_name']) ?>"><?= e($item['original_name']) ?></div>
                  <div class="meta-m-sub"><?= $sizeKb ?></div>
                </div>
              </div>
              <?php endforeach; ?>
            </div>
          ...
          <div class="pg">
            ...
            <?php for ($p = max(1,$page-2); $p <= min($totalPages,$page+2); $p++): ?>
              ...
              <?php else: ?><a href="?page=<?= $p ?>&search=<?= urlencode($search) ?><?= $typeFilter && $typeFilter!=='all' ? '&type='.e($typeFilter) : '' ?>"><?= $p ?></a><?php endif; ?>
            <?php endfor; ?>
...
<div class="modal-back" id="deleteModal">
  <div class="modal-box">
    ...
    <div class="modal-actions">
      <button class="btn btn-danger" style="flex:1" onclick="confirmDelete()">Smazat</button>
      <button class="btn btn-ghost" style="flex:1" onclick="closeModal()">Zrušit</button>
    </div>
  </div>
</div>
...
function refreshSel() {
  ...
  if (selbar) selbar.classList.toggle('show', n > 0);
}

if (grid) {
  grid.addEventListener('click', e => {
    const card = e.target.closest('.card-m');
    if (card) { card.classList.toggle('sel'); refreshSel(); }
  });
}

document.getElementById('selClear')?.addEventListener('click', () => {
  getSelected().forEach(c => c.classList.remove('sel')); refreshSel();
});

// Type filter (client-side)
const typeFilter = '<?= e($typeFilter) ?>';
if (typeFilter && typeFilter !== 'all' && grid) {
  grid.querySelectorAll('.card-m').forEach(c => {
    if (c.dataset.type !== typeFilter) c.style.display = 'none';
  });
}

function openDeleteModal() {
  document.getElementById('deleteCount').textContent = getSelected().length;
  document.getElementById('deleteModal').classList.add('show');
}
function closeModal() { document.getElementById('deleteModal').classList.remove('show'); }

async function confirmDelete() {
  const ids = getSelected().map(c => c.dataset.id);
  const fd = new FormData();
  fd.append('ajax_action', 'delete_media');
  ids.forEach(id => fd.append('ids[]', id));
  try {
    const r = await fetch(location.href, { method: 'POST', body: fd });
    const data = await r.json();
    if (data.success) location.reload();
    else { alert('Chyba: ' + data.message); closeModal(); }
  } catch { closeModal(); }
}
...
uploadStrip?.addEventListener('click', e => { if (e.target === uploadStrip || e.target.closest('.upload-ico') || e.target.closest('.upload-text') || e.target.closest('.upload-sub')) uploadInput.click(); });

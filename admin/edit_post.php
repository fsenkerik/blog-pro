        ...
        <div class="ed-grid">
          ...
          <div class="ed-side">
            ...
            <div class="sp"><div class="seo-head"><div class="sp-title"><svg class="ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg>SEO &amp; sdílení</div></div><div class="sp-body">
              <div class="field"><div class="field-label">SEO Title</div><input type="text" class="field-input" id="seoTitle" value="<?= e($currentMetaTitle) ?>" placeholder="Ponechte prázdné pro titulek…" maxlength="80"><div class="field-foot"><span>Optimum 50–60 znaků</span><span id="seoTitleCount" class="ok"><?= strlen($currentMetaTitle) ?> / 60</span></div></div>
              <div class="field"><div class="field-label">Meta Description</div><textarea class="field-textarea" id="seoDesc" placeholder="Krátký popis…" maxlength="200"><?= e($currentMetaDesc) ?></textarea><div class="field-foot"><span>Optimum 150–160 znaků</span><span id="seoDescCount" class="ok"><?= strlen($currentMetaDesc) ?> / 160</span></div></div>
              <div class="field"><div class="field-label">Klíčová slova</div><input type="text" class="field-input" id="seoKeywords" value="<?= e($currentMetaKw) ?>" placeholder="slovo 1, slovo 2…"></div>
...
document.getElementById('featuredInput')?.addEventListener('change',function(){if(this.files[0]){const r=new FileReader();r.onload=e=>{let p=document.getElementById('featPreview');if(!p){const dz=document.getElementById('dropzone');p=document.createElement('div');p.className='feat-preview';p.id='featPreview';p.innerHTML='<img src="" alt=""><button type="button" class="feat-remove" onclick="removeFeatured()"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>';dz?.replaceWith(p);}p.querySelector('img').src=e.target.result;};r.readAsDataURL(this.files[0]);markChanged();}});
</script>
</body>
</html>